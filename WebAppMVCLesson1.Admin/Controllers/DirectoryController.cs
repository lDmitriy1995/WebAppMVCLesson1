...
namespace WebAppMVCLesson1.Admin.Controllers
{
    public class DirectoryController : Controller
    {
        ...
        private WebAppMVCLesson1DbContext context;
        public DirectoryController(IWebHostEnvironment _hostEnvironment,IConfiguration _config,IOptions<customOptionsConfiguration> _options,
            WebAppMVCLesson1DbContext _context)
        {
        ...
        {
            var optin1 = config.GetValue<string>("customOptions:complexOption:optin1");

            var optin2 = config.GetSection("customOptions").GetSection("complexOption").GetSection("optin2").Value;

            customOptionsConfiguration coc = new customOptionsConfiguration(); config.GetSection("").Bind(coc);
